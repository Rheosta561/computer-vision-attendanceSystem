...
import { z } from 'zod';
import { createEvent, getEvent, searchEvents, updateEvent, deleteEvent, } from '@/controllers/event.controller';
...
// get gropu events 
eventRouter.get("/group/my", verifyJWT, async (c) => {
    try {
        const user = c.get("user");
        ...
        return c.json(new ApiResponse(500, null, "Failed to fetch group events"), { status: 500 });
    }
});
// deleting the event 
...
eventRouter.get('/group/:groupId', verifyJWT, async (c) => {
    try {
        ...
        return c.json(new ApiResponse(err.statusCode ?? 500, null, err.message ?? 'Something went wrong'), err.statusCode ?? 500);
    }
});
